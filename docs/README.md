...
***NOTE:*** `Guard` event name is reserved for my personal use related to a [custom external service integration.](https://alfaexploit.com/en/posts/calendar_server/#integration-with-external-services) Please avoid its use as `icon change` functionality ignores events with that name.
